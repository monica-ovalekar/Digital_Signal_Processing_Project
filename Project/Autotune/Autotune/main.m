...
function pushbutton1_Callback(hObject, eventdata, handles)
    ...
    [filename, pathname] = uigetfile('.mid', 'Select the MIDI File');
...
function Record_Callback(hObject, eventdata, handles)
    ...
    if (get(hObject,'Value') == 1)
        set(handles.label_rec,'string','Recording');
        record(recObj);
        %play(recObj);
    else
        stop(recObj);
        set(handles.label_rec,'string','Recorded Audio.');
...
function pushbutton2_Callback(hObject, eventdata, handles)
    global audio_y;
    global audio_Fs;
    [filename, pathname] = uigetfile('.wav', 'Select the Audio Files');
...
function pushbutton5_Callback(hObject, eventdata, handles)
    ...
    audiowrite('twinkle_AutoTuned_File.wav',new_y,audio_Fs);
